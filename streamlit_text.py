...
# ============================================================
# INTERNAL MODEL FEATURES — hidden from user interface
# ============================================================
...
def estimate_repeat_counts(
    ...
    """
    Estimate repeat-unit counts using PM% or DP% as the input.

    percent_X = percentage of PM or DP.
    percent_DM is calculated internally as 100 - percent_X.
    """
    ...
    if percent_X < 0 or percent_X > 100:
        raise ValueError("PM/DP percentage must be between 0 and 100.")

    percent_DM = 100.0 - percent_X
...
def make_public_result_table(result):
    """
    Detailed user-facing output table for download.
...
def make_short_result_table(result):
    ...
    rows = [
        ...
        {
            "Property": "Input total MW",
            "Value": result["total_MW_input"]
        },
        {
            "Property": "Input DM (%)",
        ...
        {
            "Property": f"Input {result['X_type']} (%)",
...
st.write(
    "Generate a representative silicone oil structure first. "
...
with st.sidebar:
    ...
    percent_X = st.number_input(
        ...
        value=10.0,
        step=1.0,
        help="Enter PM% or DP%. DM% is calculated automatically."
    )

    calculated_DM = 100.0 - percent_X
    st.info(f"Calculated DM (%) = {calculated_DM:.2f}")
...
if st.session_state.result is not None:
    ...
    # --------------------------------------------------------
    # Representative sequence instead of SMILES
    # --------------------------------------------------------
    ...
    if st.session_state.prediction_value is not None:
        st.metric(
            ...
            f"{st.session_state.prediction_value:.4f} kPa"
        )
    ...
    prediction_summary = pd.DataFrame([{
        ...
        "Total MW": result["total_MW_input"],
        "DM (%)": result["percent_DM_input"],
        f"{result['X_type']} (%)": result["percent_X_input"],
        "Second repeat unit": result["X_type"],
        "Oil loading (%)": result["oil_loading_percent"],
        "Predicted ice adhesion (kPa)": st.session_state.prediction_value,
    }])
    ...
    d1.download_button(
        "Output CSV",
        public_csv,
        file_name=f"{result['Oil_ID']}_output_values.csv",
        mime="text/csv"
    )
    ...
    d2.download_button(
        ...
        file_name=f"{result['Oil_ID']}_prediction_summary.csv",
        mime="text/csv"
    )
    ...
    d3.download_button(
        "Sequence",
        sequence_data,
        file_name=f"{result['Oil_ID']}_sequence.txt",
        mime="text/plain"
    )
    ...
    d5.download_button(
        "MOL",
        mol_data,
        file_name=f"{result['Oil_ID']}_3D.mol",
        mime="text/plain"
    )
    ...
    d6.download_button(
        "SDF",
        sdf_data,
        file_name=f"{result['Oil_ID']}_3D.sdf",
        mime="text/plain"
    )
    ...
    d7.download_button(
        "XYZ",
        xyz_data,
        file_name=f"{result['Oil_ID']}_3D.xyz",
        mime="text/plain"
    )
